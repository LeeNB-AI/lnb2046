...
macOS：在线工作台点击“下载/连接本地助手”后，会自动复制一条终端命令。打开“终端”，粘贴后回车即可。
...
线上网页不能静默安装电脑程序，所以第一次需要用户运行这个本地助手。macOS 不再依赖双击 `.command`，避免系统提示“没有正确访问权限”。助手启动后保持终端/PowerShell 窗口打开，线上工作台会通过 `http://127.0.0.1:4789` 连接它，然后右侧面板就可以测试登录、拉取热点并导入到第 2 步。
```
